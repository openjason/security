...
def price_right(value,gap):
    if abs(gap) < (value * 0.31):
        return True
    else:
        print('Out of 30% range.')
        return False
...
def dk_detect():
    ...
    #标号 数字 显示 从 1 开始，与配置文件一致，读取配置文件标号已做处理 。
        ...
        #记录全部交易类型的日志。
        logging.info(str_time+"'"+str(new_price)+" "+str(updown_rate)+'|'+str(updown_pice)+"|"+"dk"+"_"+str(dk_amount)\
...
def is_exchage_time(i):
    ...
    if (int(str_time[9:16]) in range(82700, 173800) or int(str_time[9:16]) in range(125700, 150800)):
        return True
    else:
